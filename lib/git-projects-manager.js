...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'git-projects-manager:refresh_projects': () => this.refresh_projects(),
      'git-projects-manager:toggle': () => this.toggle(),
      'git-projects-manager:open_project': (project) => this.open_project(project)
    }));
  ...
  refresh_projects() {
    ...
    if (file_utils.exists(basePath)) {
      ...
      for (const a_project in our_projects){
        ...
        //Creating open_project commands dinamically
        const command_to_append = 'git-projects-manager:open_project_' + a_project;
        this.subscriptions.add(atom.commands.add('atom-workspace', {
        ...
        our_submenus.submenu[0].submenu.push(
          {
            label: a_project,
            command: 'git-projects-manager:open_project_' + a_project,
          }
        )
