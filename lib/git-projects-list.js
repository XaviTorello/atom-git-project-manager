...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'git-projects-list:refresh_projects': () => this.refresh_projects(),
  ...
  refresh_projects() {
    ...
    if (file_utils.exists(basePath)) {
      ...
      for (const a_project in our_projects){
        ...
        const command_to_append = 'git-projects-list:open_project_' + a_project;
        this.subscriptions.add(atom.commands.add('atom-workspace', {
        ...
        our_submenus.submenu[0].submenu.push(
          {
            label: a_project,
            command: 'git-projects-list:open_project_' + a_project,
          }
        )
